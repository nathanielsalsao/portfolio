    ...
    return (
      <div
        ...
        className={`
          ...
          /* Switch to absolute only on Desktop */
          xl:absolute
          xl:top-1/2
          xl:-translate-y-1/2
          xl:mx-0
          ...
          ${isRight ? "xl:right-10" : "xl:left-10"}
          ${className}
        `}
      >
        <div
          className={`
            group relative bg-black/20 backdrop-blur-[50px]
            border border-white/20 rounded-[2.5rem]
            p-8 md:p-9 space-y-10 md:space-y-12
            shadow-[0_40px_80px_-15px_rgba(0,0,0,0.7)]
        ...
        >
          ...
          {items.map((item, index) => (
            <div key={index} className="relative z-10 space-y-2">
              <p className="font-mono text-[11px] uppercase tracking-[0.6em] text-white/40 font-bold">
                {item.label}
              </p>
              <p className="text-2xl md:text-3xl font-black tracking-tighter text-white drop-shadow-[0_2px_10px_rgba(0,0,0,0.5)]">
                {item.value}
              </p>
            </div>
          ))}
